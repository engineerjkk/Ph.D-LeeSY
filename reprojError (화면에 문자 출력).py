...
keys = [130,255,111,630, 256, 3154, 3125, 3122, 3118, 2286, 1167, 3099, 1746, 1173, 3095, 276, 2121]
...
for i in range(len(keys)):
    ...
    for str_points in tar_points_strings:
        ...
        point3D_id = int(list_str[2])#3D 포인트 좌표에 대한 ID를 가진다.
        tar_points_nums.append([tmp1, tmp2, point3D_id])

        tar_pt_2ds.append([tmp1, tmp2])

        tar_3dPt = hloc_points3Ds[point3D_id]#이게 iD를 받으니까 이게 3D 좌표지.
        print("\ntar_3dPt.xyz : ",tar_3dPt.xyz)
        #tar_3dPt.xyz = np.concatenate(tar_3dPt.xyz).tolist() #차원수 제거
        #[*tar_3dPt.xyz]=int*tar_3dPt.xyz
        print("\n*tar_3dPt.xyz: ", *tar_3dPt.xyz)#하나의 3D 좌표
        tar_pt_3ds.append([*tar_3dPt.xyz])#이건 이 3D 좌표의 x,y,z가 퇼테고,!
        #print("\nbefore tar_pt_3ds ",tar_pt_3ds )#모든 3D 좌표 모음

        #print("\nafter tar_pt_3ds ", tar_pt_3ds)
        #tar_pt_3ds = list(map(int, tar_pt_3ds))
        #print("\nint after tar_pt_3ds ", tar_pt_3ds)
        print("\nx좌표",int(tmp1))
        print("\ny좌표",int(tmp2))
        #이미지에 포인트 찍기.
        image=cv2.circle(image, (int(tmp1), int(tmp2)), 1, (0,255,0), -1)
        tar_quat = np.array(tar_image_header[1:5])#쿼터니온
        rvec=qvec2rotmat(tar_quat)#ROTATION MATRIX 이게 걱정되는데,,
        tar_trans = np.array(tar_image_header[5:8])#위치
        ...
        imgpoints2 = np.concatenate(imgpoints2).tolist() #차원수 제거
        imgpoints2 = np.concatenate(imgpoints2).tolist()  # 차원수 제거
        print("\nimgpoints2 : ",imgpoints2)
        tem3 = float(imgpoints2[0])
        tmp4 = float(imgpoints2[1])
        image = cv2.circle(image, (int(tem3), int(tmp4)), 1, (0, 0, 255), -1)#reprojection 찍기
        error=cv2.norm((int(tmp1), int(tmp2)),(int(tem3), int(tmp4)),cv2.NORM_L2)
        print("reprojection error :",error)
        #print("type(reprojection error) :", type(error))
        mean_error+=error#에러는 길이단위가 아니라 픽셀단위이기때문에 상대적값으로 보아도 된다.

    print("len(tar_pt_3ds), ",len(tar_pt_3ds))
    #print("type(len(tar_pt_3ds)), ", float(len(tar_pt_3ds)))
